...
function addlist (obj, path) {
  let node = document.getElementById(obj);
  let string = `<div class="scanSmall">
                            <div>
                                <img class="CameraPhoto"  src="file://${path}" style="width: 100%;height: 100%;" onclick="showpicture('${path}')" />
                            </div>
                            <p>
...
function configureDevInfo (names) {
  ...
  if (isOpenCarmera) {
    objSelect.options[objSelect.length] = new Option('摄像头', '1');

  } else {
    objSelect.options[objSelect.length] = new Option('高拍仪', '0');
    var jsonObj = { FuncName: 'camOpenDev', argument: { devIndex: 0, subtype: 0, width: 0, height: 0 } };
    sendWsMessage(jsonObj);
  }
  isOpenCarmera = false;
...
function Capture () {
  ...
  var filepath = saveText + time.getYear() + time.getMonth() + time.getDate() + time.getDate() + time.getTime() + '.jpg';
  var jsonObj = { FuncName: 'camCaptureImageFile', argument: { filePath: filepath } };
  sendWsMessage(jsonObj);

  // if (t1 == 0) {
   	timer = setTimeout(function () { addlist('highMeterPhoto', filepath); }, 2000);
  // }
...
function RecToFile (type) {
  ...
  if (type == 1) {
    var jsonObj = { FuncName: 'ocrCombineSingleToFile', argument: { strImagePath: OCR_Pathtext.value, strFilePath: ocrFile, language: parseInt(languageID) } };
    sendWsMessage(jsonObj);
  } else if (type == 2) {
...
function InitSign () {
  var jsonObj = { FuncName: 'StartSignDevice' };
  sendWsMessage(jsonObj);
}
