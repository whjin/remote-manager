    ...
    <div :class="classes" v-click-outside="handleClose">
        ...
        <transition name="transition-drop">
            <Drop
                ...
                <div @scroll="scrollContent($event)">
                    <Caspanel
                    ...
                    <div :class="[prefixCls + '-dropdown']" v-show="filterable && query !== '' && querySelections.length">
                        <ul :class="[selectPrefixCls + '-dropdown-list']">
                            <li :class="[selectPrefixCls + '-item', {[selectPrefixCls + '-item-disabled']: item.disabled}]"
                              ...
                              <div class="codeBox" style="display: inline-block;" v-html="item.code"></div>
                              <div class="labelBox" style="display: inline-block;" v-html="item.display"></div>
                            </li>
                        </ul>
                    </div>
                  <ul v-show="filterable && query !== '' && !querySelections.length" :class="[prefixCls + '-not-found-tip']"><li>搜索结果为空</li></ul>
                  <ul v-show="!data.length" :class="[prefixCls + '-not-found-tip']"><li>{{ localeNotFoundText }}</li></ul>
                </div>
                <div class="btnBox">
                  <span @click="clearSelect">重新选择</span>
...
export default {
  ...
  data () {
    return {
      ...
      pageIndex: 10,
      pageEvent: {},
      searchLastValueParentThis: this.searchLastValueParent,
      scrollEvent: {}
    };
  },
  computed: {
  ...
  methods: {
      ...
      if (!this.changeOnSelect || init || changeOnSelectDataChange) {
        if (this.searchItem.children) {
          ...
          this.broadcast('Caspanel', 'on-find-selected', {
            value: this.currentValue,
            item: this.searchItem,
            oldVal: this.searchLastValueParentThis
          });
          this.searchLastValueParentThis = {};
        }
      }
    },
    ...
    handleInput (event, scroll = false) {
      if (!scroll) {
        // 只要搜索数据变化，滚动条到顶部，请求的条件初始化
        if (this.scrollEvent.target) {
          this.scrollEvent.target.scrollTo(0, 0);
        }
        this.pageIndex = 10;
        this.scrollIndex = 1;
      }
      ...
      if (event.target) {
        this.searchBoxDataUrl(4, event.target.value, this.typeCode, { pageIndex: 1, pageSize: this.pageIndex }).then(res => {
          this.query = event.target.value;
          that.querySelections.length = 0;
          let selections = [];
          ...
          that.querySelections = selections;
        });
      }
    },
    handleSelectItem (index) {
      ...
      delete this.searchItem.__label;
      delete this.searchItem.__value;
      ...
      if (this.searchItem.children) {
        this.$emit('searchLastValueEnable', false, this.searchItem);
        setTimeout(() => {
          this.emitValue(this.currentValue, oldVal);
        }, 10);
      } else {
        // 传当时的状态是否是搜索的最后一级
        this.$emit('searchLastValueEnable', true, this.searchItem);
        let timer = setInterval(() => {
          if (JSON.stringify(this.searchLastValueParentThis).length !== 2) {
            this.emitValue(this.currentValue, oldVal);
            // this.handleClose();
            clearInterval(timer);
          }
    ...
    scrollContent (ev) {
      ...
      if (ev.target.scrollHeight <= ev.target.scrollTop + ev.target.clientHeight) {
        this.scrollIndex++;
        this.pageIndex = this.scrollIndex * this.pageIndex;
        this.handleInput(this.pageEvent, true);
      }
    }
  },
  created () {
  ...
  watch: {
    ...
    searchLastValueParent (val) {
      this.searchLastValueParentThis = val;
    },
    submitClear () {
      this.clearSelect();
    }
